...
def getRatesOxyCO2(convertedList: list) -> tuple:
    oxyRate = getOxyRate(convertedList)
    cO2Rate = getCO2Rate(convertedList)
    return (int(oxyRate, 2), int(cO2Rate, 2))

def getOxyRate(formattedList: list, currentColumnIndex = 0) -> str:
    ...
    for j in range(length):
        if formattedList[j][currentColumnIndex] == "1":
            amountOfOnes += 1
    if amountOfOnes >= length / 2:
        ...
        winnerBit = "0"
    for j in range(length):
        if formattedList[j][currentColumnIndex] == winnerBit:
            nextList.append(formattedList[j])
    return getOxyRate(nextList, currentColumnIndex + 1)

def getCO2Rate(formattedList: list, currentColumnIndex = 0) -> str:
    length = len(formattedList)
    if length == 0:
        return None
    if length == 1:
    ...
    winnerBit = ""
    for j in range(length):
        if formattedList[j][currentColumnIndex] == "0":
            amountOfZeros += 1
    if amountOfZeros <= length / 2:
        winnerBit = "0"
    else:
        winnerBit = "1"
    for j in range(length):
        if formattedList[j][currentColumnIndex] == winnerBit:
            nextList.append(formattedList[j])
    return getCO2Rate(nextList, currentColumnIndex + 1)

def main() -> int:
    loadedFile = getFile("input")
    convertedList = convertFileToList(loadedFile)
    #--- Part 1
    rotatedBitsList = saveInListByColumn(convertedList)
    rates = getRates(rotatedBitsList)
    print(rates[0] * rates[1])
    #--- Part 2
    rates = getRatesOxyCO2(convertedList)
    print(rates[0] * rates[1])
